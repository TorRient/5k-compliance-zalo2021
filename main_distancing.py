...
model_dir = args.dir_model_distancing
if not os.path.exists(model_dir):
    os.mkdir(model_dir)
path_mask_csv_init = model_dir + '/train_{}folds_distancing_first.csv'.format(FOLDS)
path_mask_csv_update = model_dir + '/train_{}folds_distancing.csv'.format(FOLDS)
path_train_init = args.dir_model_mask + '/train_meta_update.csv'
...
for loopp in range(1,2):
    ...
    for fold in range(0, FOLDS):
        if loopp == 1:
            if not os.path.exists(path_mask_csv_init):
                print("CREATE FOLDS")
                create_csv_mask(path_mask_csv_init, path_train_init, task_name="distancing")
    ...
    for fold in range(0, FOLDS):
        ...
        test = test[test["distancing"].isna()]
        images = args.dir_train + '/images'
    ...
    for i in range(len(sub_csv)):
        if sub_csv.loc[i, "distancing"] != 0 and sub_csv.loc[i, "distancing"] != 1:
            if final[count] != 2:
                sub_csv.loc[i, 'distancing'] = final[count]
            count += 1
        if sub_csv.loc[i, 'distancing'] == 0 or sub_csv.loc[i, 'mask'] == 0:
        # if sub_csv.loc[i, 'mask'] == 0:
            sub_csv.loc[i, '5k'] = 0
        if sub_csv.loc[i, 'distancing'] == 1 and sub_csv.loc[i, 'mask'] == 1:
            sub_csv.loc[i, '5k'] = 1

    sub_csv.to_csv(path_train_update, index=False)
    # split
    len_df = create_csv_mask(path_mask_csv_update, path_train_update, task_name="distancing")
    loopp += 1